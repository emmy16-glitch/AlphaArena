...
function Sources({ sources }: { sources: SourceStatus }) {
  return <div className="mt-6 flex flex-wrap gap-x-5 gap-y-2 border-t border-[#E7E5DE] pt-4 text-[11px] text-[#8A8A84]">{Object.entries(sources).map(([key, value]) => <span key={key} className="capitalize">{key}<b className={`ml-1 font-medium normal-case ${sourceTone(value)}`}>{value}</b></span>)}</div>;
}
    ...
    <div className="fade-up">
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[.76fr_1.24fr]">
        <section className="rounded-[26px] border border-[#E7E5DE] bg-white p-5 sm:p-6">
          ...
          <div className="mt-5"><FieldLabel>Which way?</FieldLabel><div className="flex flex-wrap gap-2">{(['LONG', 'SHORT', 'WAIT'] as Direction[]).map((item) => <SegButton key={item} active={direction === item} onClick={() => setDirection(item)}>{item}</SegButton>)}</div><p className="mt-2 text-[12px] leading-5 text-[#8A8A84]">LONG = up · SHORT = down · WAIT = no position yet.</p></div>
          <div className="mt-5"><FieldLabel>Paper risk · {risk.toFixed(1)}%</FieldLabel><input aria-label="Paper risk allocation" type="range" min="0.5" max="10" step="0.5" value={risk} onChange={(event) => setRisk(Number(event.target.value))} className="w-full accent-[#141412]" /><p className="mt-2 text-[12px] leading-5 text-[#8A8A84]">This only changes the stress test. No money is moved.</p></div>
        ...
        <section className="rounded-[26px] border border-[#D9D7CF] bg-white p-5 sm:p-6 md:p-8" aria-busy={loading}>
          {loading && <div className="flex min-h-[500px] flex-col justify-center"><MicroLabel>2 · Testing your idea</MicroLabel><h2 className="mt-4 text-[32px] font-semibold tracking-[-0.04em]">Challenging the obvious answer.</h2><div className="mt-8 space-y-3">{['Read the live Bitget Reality market', 'Compare Vibe-Trading history', 'Check macro context', 'Build the strongest bull and bear cases', 'Run transparent stress scenarios', 'Weigh evidence quality'].map((step, index) => <div key={step} className="flex items-center gap-3 border-b border-[#E7E5DE] py-3"><span className={`h-2 w-2 rounded-full ${index < 2 ? 'bg-[#0D7A4F]' : 'shimmer-bar'}`} /><span className="text-[13px]">{step}</span></div>)}</div></div>}
          ...
          {!loading && report && <>
            <div className="flex items-start justify-between gap-4"><div><MicroLabel>3 · NightWatch result</MicroLabel><div className="mt-2 text-[12px] text-[#8A8A84]">Evidence confidence {report.confidence}% · {report.risk_level} risk</div></div><VerdictPill tone={report.resilience >= 70 ? 'up' : report.resilience < 45 ? 'down' : 'warn'}>{report.verdict}</VerdictPill></div>
