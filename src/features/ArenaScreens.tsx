...
export function ArenaScreen({ onNav, initialSymbol = 'rNVDA', initialThesis = '', initialAiSide = 'WAIT' }: { onNav: Nav; initialSymbol?: string; initialThesis?: string; initialAiSide?: Direction }) {
  ...
  const create = async () => {
    ...
    setError('');
    try {
      const battle = await productApi.createBattle({ symbol, user_side: side, ai_side: aiSide, thesis: thesis.trim(), stake, duration_hours: duration, opponent: 'NightWatch', stated_confidence: confidence });
      await load();
