...
import JudgeExports from '../components/JudgeExports';
import DecisionTapePanel from './DecisionTapePanel';
import { FlattenNote, SessionBadge, ShadowBars } from './ShadowSession';
...
export function ArenaScreen({ onNav, initialSymbol = 'rNVDA', initialThesis = '', initialAiSide = 'WAIT' }: { onNav: Nav; initialSymbol?: string; initialThesis?: string; initialAiSide?: Direction }) {
  ...
  const create = async () => {
    ...
    setError('');
    try {
      const battle = await productApi.createBattle({ symbol, user_side: side, ai_side: aiSide, thesis: thesis.trim(), wrong_sentence: wrongSentence.trim() || undefined, risk_pct: riskPct, stake, duration_hours: duration, opponent: 'NightWatch', stated_confidence: confidence });
      await load();
  ...
  return <div className="fade-up">
    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-[.78fr_1.22fr]">
      ...
      <section className="rounded-[26px] border border-[#D9D7CF] bg-white p-6 md:p-8"><MicroLabel>Start a thesis battle</MicroLabel><div className="mt-6 grid gap-5 sm:grid-cols-2"><div><FieldLabel>Asset</FieldLabel><div className="flex flex-wrap gap-2">{assets.slice(0, 6).map((asset) => <SegButton key={asset.symbol} active={symbol === asset.symbol} onClick={() => setSymbol(asset.symbol)}>{asset.name || asset.symbol}</SegButton>)}</div></div><div><FieldLabel>Your side</FieldLabel><div className="flex flex-wrap gap-2">{(['LONG', 'SHORT', 'WAIT'] as Direction[]).map((item) => <SegButton key={item} active={side === item} onClick={() => setSide(item)}>{directionLabel(item)}</SegButton>)}</div></div></div><div className="mt-5"><FieldLabel>NightWatch stance</FieldLabel><div className="flex flex-wrap gap-2">{(['LONG', 'SHORT', 'WAIT'] as Direction[]).map((item) => <SegButton key={item} active={aiSide === item} onClick={() => setAiSide(item)}>{directionLabel(item)}</SegButton>)}</div></div><div className="mt-5"><FieldLabel>Thesis</FieldLabel><textarea aria-label="Battle thesis" value={thesis} onChange={(event) => setThesis(event.target.value)} className="min-h-[110px] w-full resize-none rounded-2xl border border-[#D9D7CF] bg-[#FAF9F6] p-4 text-[13px] leading-5 outline-none focus:border-[#141412]" /></div><div className="mt-5"><FieldLabel>If I am wrong, it will be because…</FieldLabel><input aria-label="If I am wrong, it will be because" value={wrongSentence} onChange={(event) => setWrongSentence(event.target.value)} maxLength={500} placeholder="e.g. weekend tape gaps against me while NYSE is closed" className="w-full rounded-2xl border border-[#D9D7CF] bg-[#FAF9F6] p-4 text-[13px] leading-5 outline-none focus:border-[#141412]" /><p className="mt-2 text-[11px] leading-5 text-[#8A8A84]">Locked at entry, hashed into the freeze, read back verbatim. No editing after this point.</p></div><div className="mt-5 grid gap-5 sm:grid-cols-2"><div><FieldLabel>Kill level · {riskPct.toFixed(1)}% adverse move</FieldLabel><input aria-label="Kill level percent" type="range" min={0.5} max={10} step={0.5} value={riskPct} onChange={(event) => setRiskPct(Number(event.target.value))} className="w-full accent-[#141412]" /></div><div><FieldLabel>Virtual stake · {money(stake)}</FieldLabel><input aria-label="Virtual stake" type="range" min="1000" max={Math.max(1000, Math.min(50000, Math.floor(freeCapital / 1000) * 1000))} step="1000" value={Math.min(stake, Math.max(1000, freeCapital))} onChange={(event) => setStake(Number(event.target.value))} className="w-full accent-[#141412]" /></div><div><FieldLabel>Duration</FieldLabel><div className="flex gap-2">{[6, 24, 72].map((hours) => <SegButton key={hours} active={duration === hours} onClick={() => setDuration(hours)}>{hours < 24 ? `${hours}H` : `${hours / 24}D`}</SegButton>)}</div></div></div><div className="mt-5"><FieldLabel>Stated confidence · {confidence}% likely to win</FieldLabel><input aria-label="Stated confidence" type="range" min="10" max="95" step="5" value={confidence} onChange={(event) => setConfidence(Number(event.target.value))} className="w-full accent-[#141412]" /><p className="mt-1 text-[11px] leading-5 text-[#8A8A84]">Your honest probability, recorded before entry. The Track Record page later checks it against outcomes.</p></div><ActionButton onClick={() => void create()} className="mt-6">{creating ? 'Recording battle…' : 'Enter the Arena'} <ArrowRight size={14} /></ActionButton>{error && <p className="mt-3 text-[11px] text-[#C93A3A]">{error}</p>}</section>
    </div>

    <DecisionTapePanel symbol={symbol} thesis={thesis} thesisDirection={side} riskPct={riskPct} />

    <div className="mt-10 flex items-center justify-between"><h2 className="text-[20px] font-semibold">Your battles</h2><JudgeExports /><button onClick={() => void load()} className="flex items-center gap-1.5 text-[11px] text-[#55554F]"><RefreshCw size={12} />Refresh</button></div>
    {battles.length === 0 ? <div className="mt-4 rounded-[22px] border border-dashed border-[#D9D7CF] p-8 text-center text-[13px] text-[#8A8A84]">No recorded battles yet. Your first one will appear here.</div> : <div className="mt-4 grid gap-4 md:grid-cols-3">{battles.map((battle) => <button key={battle.id} onClick={() => onNav('battle', { id: battle.id })} className="rounded-[22px] border border-[#E7E5DE] bg-white p-5 text-left transition-all hover:-translate-y-0.5 hover:border-[#D9D7CF]"><div className="flex items-center justify-between"><span className="font-semibold">{battle.symbol}</span><div className="flex items-center gap-2">{battle.shadow?.kill_session && battle.shadow?.kill_hit === true && <SessionBadge session={battle.shadow.kill_session} />}{battle.shadow?.kill_hit === true ? <VerdictPill tone="down">killed</VerdictPill> : <VerdictPill tone={battle.status === 'live' ? 'up' : 'neutral'}>{battle.status}</VerdictPill>}</div></div><p className="mt-4 min-h-[58px] text-[12px] leading-5 text-[#55554F]">{battle.thesis}</p><Hairline className="my-4" /><div className="flex items-center justify-between"><span className="text-[11px]">{battle.user_side} vs {battle.ai_side}</span><Pnl value={battle.user_pnl_pct} className="text-[11px]" /></div></button>)}</div>}
  </div>;
}
...
export function BattleScreen({ onNav, battleId }: { onNav: Nav; battleId?: string }) {
  ...
  const checkFreeze = async () => {
    if (!battle || battle.status !== 'settled' || verifying) return;
    setVerifying(true);
    try {
      const result = await productApi.verifyBattle(battle.id);
      setVerified(result.verified);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Verify unavailable'); }
