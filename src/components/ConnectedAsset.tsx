...
import { ActionButton, Hairline, MicroLabel, Pnl, Sparkline, VerdictPill } from './ui';
...
export default function ConnectedAsset({ symbol, onNav }: { symbol: string; onNav: Nav }) {
  ...
  useEffect(() => {
    ...
    void productApi.pulse().then((events)=>{
      if (alive) setPulse(events.find((e)=>e.symbol===asset.symbol) || null);
    }).catch(()=>{});
    return () => { alive = false; };
  }, [asset.symbol]);

  const metrics = useMemo(() => {
    const values = asset.spark.filter((v)=>v>0);
    const changes = values.slice(1).map((v,i)=>((v/values[i])-1)*100);
    ...
    <div className="fade-up">
      <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <div className="mt-3 flex flex-wrap items-baseline gap-x-4 gap-y-2">
            <h1 className="text-[48px] font-semibold tracking-[-0.055em] md:text-[64px]">{asset.symbol}</h1>
            <span className="text-[16px] text-[#8A8A84]">{asset.name}</span>
          </div>
      ...
      <section className="mt-8 overflow-hidden rounded-[28px] border border-[#D9D7CF] bg-white">
        ...
        <div className="grid grid-cols-2 border-t border-[#E7E5DE] md:grid-cols-4">
          {[
            ['24H high', `$${fmtPrice(asset.high24)}`],
            ['24H low', `$${fmtPrice(asset.low24)}`],
            ['24H volume', asset.volume],
            ['24H range', `${metrics.range.toFixed(2)}%`],
          ].map(([label,value])=><div key={label} className="border-r border-[#E7E5DE] p-4 last:border-r-0"><MicroLabel>{label}</MicroLabel><div className="mono-num mt-2 text-[17px] font-semibold">{value}</div></div>)}
        </div>
      </section>
