...
import { Activity, Crosshair, FlaskConical, Skull, Swords, Wallet, Trophy, Plus, ChevronLeft, ShieldCheck } from 'lucide-react';
import { Logo } from './ui';
...
const nav = [
  ...
  { id: 'morgue', label: 'Morgue', icon: Skull, hint: 'Dead theses' },
    ...
    <div className="min-h-screen bg-[#FAF9F6]">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[232px] flex-col border-r border-[#E7E5DE] bg-[#FAF9F6]/95 backdrop-blur-xl lg:flex">
        ...
        <div className="mx-3 rounded-2xl border border-[#E7E5DE] bg-white p-3.5">
          ...
          <p className="mt-1.5 text-[11.5px] leading-5 text-[#8A8A84]">No wallet, deposit, or real-money trading is connected.</p>
        </div>
        ...
        <nav aria-label="Main navigation" className="mt-5 flex-1 space-y-1 px-3">
          ...
          <button onClick={onCreate} className={cn('flex min-h-11 w-full items-center gap-3 rounded-xl border border-dashed px-3 text-[14px] font-medium transition-colors', view === 'create' ? 'border-[#141412] bg-[#141412] text-white' : 'border-[#D9D7CF] text-[#55554F] hover:border-[#141412] hover:text-black')}><Plus size={17} /> AI challengers</button>
        </nav>
      ...
      <header className="sticky top-0 z-40 border-b border-[#E7E5DE] bg-[#FAF9F6]/92 backdrop-blur-xl lg:ml-[232px]">
        <div className="flex h-[64px] items-center justify-between px-4 md:px-8">
          ...
          <button onClick={() => onNav('portfolio')} className="flex min-h-11 items-center gap-2.5 rounded-full border border-[#E7E5DE] bg-white py-1 pl-1 pr-3.5 transition-colors hover:border-[#141412]" aria-label={`Open guest paper portfolio, ${compactMoney(net)}`}>
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#141412] text-[9px] font-semibold text-white">GUEST</span>
            <span className="mono-num hidden text-[12.5px] font-semibold sm:inline">{compactMoney(net)}</span>
            <span className="hidden text-[11px] text-[#8A8A84] md:inline">virtual</span>
          </button>
        </div>
      </header>
      ...
      <nav aria-label="Mobile navigation" className="fixed inset-x-0 bottom-0 z-40 border-t border-[#E7E5DE] bg-[#FAF9F6]/96 backdrop-blur-xl lg:hidden">
        <div className="grid grid-cols-7 px-1 pb-[max(6px,env(safe-area-inset-bottom))] pt-1.5">
          {nav.map((n) => {
