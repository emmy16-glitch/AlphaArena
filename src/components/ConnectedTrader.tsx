...
export default function ConnectedTrader({ onNav }: { onNav: Nav }) {
  ...
  return (
    <div className="fade-up">
      <MicroLabel>AI challengers · Virtual only</MicroLabel>
      <div className="mt-3 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-[650px] text-[14px] leading-6 text-[#55554F]">Build an AI challenger for paper-only battles. Choose how it thinks and how much uncertainty it is willing to tolerate.</p>
        </div>
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
        <section className="rounded-[26px] border border-[#E7E5DE] bg-white p-6 md:p-8">
          ...
          <ActionButton onClick={()=>void save()} className="mt-8">{saving?'Saving…':'Save AI challenger'} <ArrowRight size={14}/></ActionButton>
        ...
        <aside className="rounded-[26px] bg-[#141412] p-6 text-white md:p-8">
          <MicroLabel className="text-white/45">Challenger preview</MicroLabel>
          ...
          <div className="mt-8 flex gap-2 text-[11px] leading-5 text-white/50"><Shield size={15} className="mt-0.5 shrink-0"/><span>Virtual-only by design. This challenger can argue, simulate and compete, but it cannot place real-money trades.</span></div>
