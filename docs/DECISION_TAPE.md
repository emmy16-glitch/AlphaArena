...
The 5-second-style loop from the Bitget/Jev example runs as a **separate worker**, not inside Vercel request handlers:

```bash
cd backend
python scripts/decision_worker.py
```
...
JEV_ADAPTER_URL=https://your-jev-wrapper.example/decision
JEV_ADAPTER_TOKEN=...
```
...
Bitget snapshot
   ...
   └─> optional Jev adapter -> typed Jev lane
              ↓
       persisted Decision Tape
              ↓
       matured 5m/30m/1h/24h
       outcomes evaluated
```
