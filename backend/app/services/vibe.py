...
def _paired_beta(
    ...
    benchmark_returns: list[tuple[str, float]],
...
def _move_analogues(
    ...
    for index, (_, move) in enumerate(returns[:-1]):
        if (1 if move >= 0 else -1) != direction:
            continue
        candidates.append((abs(abs(move) - target), index))
        ...
        analogues.append(
            {
                ...
                "relevance": (
                    "Selected mechanically as one of the closest same-direction daily moves "
                    f"to the current {current_move_pct:+.2f}% move; it is an analogue, not a prediction."
                ),
            }
        )
    return analogues
...
class VibeTradingResearch:
    ...
    async def health(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "connected": required.issubset(names),
                "tool_count": len(names),
                "required_tools": sorted(required),
                "missing": sorted(required - names),
            }
        except Exception:
            return {"connected": False, "reason": "Vibe-Trading research is unavailable"}
    ...
    async def snapshot(self, display_symbol: str, lookback_days: int = 365) -> dict[str, Any]:
        ...
        if not self.enabled:
            return {
                ...
                "historical_stats": {},
                "analogues": [],
                "errors": ["Vibe-Trading research is not configured"],
            }
        ...
        calls: list[tuple[str, dict[str, Any]]] = [
            ...
            (
                ...
                {"code": equity_code, "statement": "indicators", "period": "quarter", "offset": 0},
            ),
        ]
        ...
        async def run(name: str, arguments: dict[str, Any]) -> tuple[str, Any, str | None]:
            try:
                value = await asyncio.wait_for(
                    ...
                    timeout=settings.mcp_timeout_seconds + 5,
                )
                return name, value, None
            except Exception:
                return name, None, "temporarily unavailable"
        ...
        current_move = asset_returns[-1][1] if asset_returns else 0.0
        ...
        self._cache[cache_key] = _CacheItem(
            result,
            time.time() + max(1, settings.vibe_cache_seconds),
        )
        return result
